...
GARCH <- function(param, eps) {
    ...
    logL <- c()
    #sigma2 <- c(omega / (1 - alpha - beta))
    sigma2 <- c(param[4])
    for (i in 1:n) {
      logL <- c(logL, dnorm(eps[i], mean=0, sd=sqrt(sigma2[i]), log=TRUE))
      sigma2 <- c(sigma2, omega + alpha * eps[i]^2 + beta * sigma2[i])
    }
    #sigma2 <- sigma2[2:n] # same length as eps
...
# WORK FOR YOU HERE
# running the NL opt (use the nlminb procedure here)
mu = mean(yield)
out <- nlminb(param, negLL, eps=yield - mu, lower=c(0, 0, 0, 0), upper=c(999, 1, 1, 999))
```
