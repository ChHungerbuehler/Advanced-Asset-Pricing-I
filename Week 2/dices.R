...
throws <- sample(1:s, n, replace=TRUE)
...
# 's' is already used and not good naming convention as not mnemonic ...

d_throws <- replicate(n, sum(sample(1:s, d, replace=TRUE)))

# plot density of the sum of d dices
density_estimate <- density(d_throws)
